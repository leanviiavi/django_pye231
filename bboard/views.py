...
def by_rubric(request, rubric_id=None):
    ...
    if rubric_id:
        ...
        """ Получаем все объявления по текущей рубрике """
        bbs = Bb.objects.filter(rubric=rubric_id)
        bbs_ = list(bbs[::])
        """ Определяем, является ли данная рубрика parent """
        parent = Rubric.objects.get(pk=rubric_id)
        if parent.parent_rubric is None:
            """ получаем сам объект рубрики """
            parent = Rubric.objects.get(pk=rubric_id)
            """ фильтром получаем все дочерние этой рубрики """
            childs = Rubric.objects.filter(parent_rubric=parent)
            """ по всем дочерним получаем объявления """
            for child in childs:
                child_bb = Bb.objects.filter(rubric=child)
                """ добавляем объявления в ответ """
                bbs_ = [*bbs_, *child_bb]
